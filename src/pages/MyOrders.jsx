...
const statusColor = {
  ...
  Pending: 'bg-amber-400/10 text-amber-600 border border-amber-400/20',
  Paid: 'bg-emerald-500/10 text-emerald-600 border border-emerald-500/20',
  Partial_Paid: 'bg-amber-400/10 text-amber-600 border border-amber-400/20',
...
function OrderDetail({ orderRef, email }) {
  ...
  if (!data) {
    return <p className="mt-4 text-xs text-rose-600">Could not load order details right now.</p>;
  }
  ...
  return (
    <div className="mt-4 pt-4 border-t border-zinc-200 space-y-4">
      <div>
        <h4 className="text-xs font-bold text-zinc-500 uppercase tracking-wider mb-2">Items</h4>
        <div className="space-y-2">
          {(data.items || []).map((item, idx) => (
            <div key={idx} className="flex justify-between items-center bg-zinc-100 px-3 py-2.5 rounded-xl text-sm">
              <div>
                <p className="font-medium text-zinc-900">{item.name || 'Product'}</p>
                <p className="text-xs text-zinc-500">Qty: {item.quantity}</p>
              </div>
              <p className="font-semibold text-amber-600">{formatINR(item.price_at_purchase * item.quantity)}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      {data.tracking_available && (
        <div>
          <h4 className="text-xs font-bold text-zinc-500 uppercase tracking-wider mb-2 flex items-center gap-1.5">
            <Truck size={13} className="text-amber-600" /> Shipment Tracking
          </h4>
          {data.courier_status === 'UNAVAILABLE' ? (
            <p className="text-xs text-amber-600/80">Live courier status is temporarily unavailable. Please check back shortly.</p>
          ) : scans.length === 0 ? (
...
export default function MyOrders() {
  ...
  if (!user) {
    return (
      <div className="min-h-screen bg-white text-zinc-900 flex items-center justify-center pt-28 font-sans px-6 text-center">
  ...
  return (
    <div className="min-h-screen bg-white text-zinc-900 px-4 md:px-10 pt-28 md:pt-36 pb-20 font-sans">
      <div className="max-w-4xl mx-auto">
        <h1 className="text-2xl md:text-3xl font-black mb-8 flex items-center gap-3">
          <Package className="text-amber-600" size={26} /> Your Orders
        </h1>
        ...
        {loading ? (
          ...
          <div className="space-y-4">
            {orders.map((o) => {
              ...
              return (
                <div key={o.order_id} className="bg-white border border-zinc-200 rounded-2xl p-5">
                  <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-3">
                    <div className="flex flex-wrap items-center gap-x-6 gap-y-2">
                      <div>
                        <p className="text-[10px] text-zinc-500 uppercase tracking-wider">Order</p>
                        <p className="font-bold text-sm text-zinc-900 font-mono">{o.razorpay_order_id}</p>
                      </div>
                      <div>
                        <p className="text-[10px] text-zinc-500 uppercase tracking-wider">Date</p>
                        <p className="text-sm text-zinc-600">{new Date(o.created_at).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })}</p>
                      </div>
                      <div>
                        <p className="text-[10px] text-zinc-500 uppercase tracking-wider">Total</p>
                        <p className="text-sm font-bold text-amber-600">{formatINR(o.full_amount)}</p>
                      </div>
                      <span className={`text-[10px] font-bold uppercase px-2.5 py-1 rounded-full ${statusColor[o.status] || 'bg-zinc-200 text-zinc-600'}`}>
                        {o.status}
                      </span>
                    </div>
                    <button
                      ...
                      className="text-amber-600 text-xs font-bold flex items-center gap-1 cursor-pointer shrink-0"
                    >
