  ...
  return (
    <div className="min-h-screen bg-white text-slate-100 font-sans selection:bg-amber-400 selection:text-black">
        ...
        {items.map((item, idx) => {
          ...
          return (
            <a
              ...
              className="flex items-center justify-between gap-4 bg-white border border-zinc-200 rounded-2xl p-6 hover:border-amber-400/40 transition-colors group"
            >
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 rounded-xl bg-amber-400/10 border border-amber-400/20 flex items-center justify-center text-amber-600 shrink-0">
                  <Icon size={22} />
                </div>
                <div>
                  <p className="text-zinc-900 font-bold text-sm group-hover:text-amber-600 transition-colors">{item.title}</p>
                  <p className="text-zinc-500 text-xs uppercase tracking-wide mt-0.5">{item.type}</p>
                </div>
              </div>
              <Download size={18} className="text-zinc-500 group-hover:text-amber-600 transition-colors shrink-0" />
            </a>
          );
        })}
      </div>
      ...
      <div className="max-w-4xl mx-auto px-6 pb-20 text-center text-xs text-zinc-500">
        ...
        <a href="/contact" className="text-amber-600 hover:underline">Customer Care</a> page.
      </div>
    </div>
  );
}
