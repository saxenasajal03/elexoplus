  ...
  return (
    <div className="min-h-screen bg-white text-slate-100 font-sans selection:bg-amber-400 selection:text-black">
      ...
      <div className="max-w-xl mx-auto px-6 py-16">
        <form onSubmit={handleVerify} className="bg-white border border-zinc-200 rounded-2xl p-6 space-y-4">
          <div className="flex items-center gap-2 text-xs text-zinc-500 mb-1">
            <ScanLine size={14} className="text-amber-600" /> Found on the QR sticker on your product body
          </div>
          <input
            type="text"
            required
            ...
            className="w-full bg-zinc-100 border border-zinc-200 rounded-xl px-4 py-3.5 text-sm text-zinc-900 placeholder-zinc-400 focus:outline-none focus:ring-2 focus:ring-amber-400 uppercase"
          />
          <button
            type="submit"
            disabled={loading}
        ...
        {error && (
          <div className="mt-6 bg-white border border-amber-500/20 rounded-2xl p-5 flex items-start gap-3">
        ...
        {result && (
          <div className={`mt-6 rounded-2xl p-6 border ${result.verified ? 'bg-emerald-500/5 border-emerald-500/30' : 'bg-rose-500/5 border-rose-500/30'}`}>
            <div className="flex items-center gap-3">
              {result.verified ? <CheckCircle2 className="text-emerald-600" size={24} /> : <AlertTriangle className="text-rose-600" size={24} />}
              <p className={`font-extrabold text-lg ${result.verified ? 'text-emerald-600' : 'text-rose-600'}`}>
                {result.verified ? 'Genuine ElexoPlus Product' : 'Could Not Verify This Serial'}
              </p>
            </div>
            {result.verified && (
              <div className="mt-4 grid grid-cols-2 gap-3 text-xs">
                <div><p className="text-zinc-500">Product</p><p className="text-zinc-900 font-bold">{result.product_name || '—'}</p></div>
                <div><p className="text-zinc-500">Model</p><p className="text-zinc-900 font-bold">{result.model || '—'}</p></div>
                <div><p className="text-zinc-500">Batch</p><p className="text-zinc-900 font-bold">{result.batch || '—'}</p></div>
                <div><p className="text-zinc-500">Manufactured</p><p className="text-zinc-900 font-bold">{result.mfg_date || '—'}</p></div>
              </div>
            )}
            {!result.verified && (
