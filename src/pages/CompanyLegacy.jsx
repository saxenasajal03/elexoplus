  ...
  return (
    <div className="min-h-screen bg-white text-slate-100 font-sans selection:bg-amber-400 selection:text-black">
      ...
      <div className="max-w-4xl mx-auto px-6 py-16 md:py-20">
        <div className="relative border-l-2 border-zinc-200 ml-3 md:ml-0 md:pl-0">
          {milestones.map((m, idx) => (
            <div key={m.id ?? idx} className="relative pl-8 md:pl-12 pb-12 last:pb-0">
              ...
              <div className="bg-white border border-zinc-200 rounded-2xl p-6 hover:border-amber-400/40 transition-colors">
                <span className="text-amber-600 font-black text-2xl md:text-3xl tracking-tight">{m.year}</span>
                <h3 className="text-zinc-900 font-extrabold text-lg mt-2">{m.title}</h3>
                <p className="text-zinc-500 text-sm mt-2 leading-relaxed">{m.description}</p>
              </div>
            </div>
          ))}
        </div>
