...
export default function HeroSection() {
  ...
  return (
    <div className="relative bg-gradient-to-br from-amber-50 via-white to-orange-50 text-zinc-900 overflow-hidden font-sans border-b border-zinc-200 pt-4">
      {/* Background Glow Elements */}
      <div className="absolute top-1/4 left-1/10 w-96 h-96 bg-amber-300/20 rounded-full blur-3xl pointer-events-none"></div>
      <div className="absolute bottom-1/4 right-1/10 w-96 h-96 bg-orange-300/20 rounded-full blur-3xl pointer-events-none"></div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-12 md:py-20 grid grid-cols-1 md:grid-cols-2 items-center gap-12 relative z-10 min-h-[480px]">
        ...
        <div className="space-y-6 transition-all duration-500 ease-in-out">
          <br></br><br></br>
          <div className="inline-flex items-center gap-2 bg-amber-400/15 text-amber-700 border border-amber-400/30 px-3.5 py-1.5 rounded-full text-xs font-extrabold uppercase tracking-wider">
          ...
          <p className="text-zinc-600 text-sm md:text-base max-w-lg leading-relaxed font-medium">
            {slide.description}
          </p>
          ...
          <div className="flex flex-wrap gap-4 pt-2">
            ...
            <Link 
              ...
              className="bg-white hover:bg-zinc-50 text-zinc-900 border border-zinc-300 px-8 py-3.5 rounded-full font-extrabold transition text-sm flex items-center gap-2 shadow-sm"
            >
              <Zap size={16} className="text-amber-600" /> {slide.secondaryCta.text}
            </Link>
          </div>

          <div className="flex items-center gap-6 pt-6 text-xs text-zinc-500 border-t border-zinc-200">
            <span className="flex items-center gap-1.5 font-bold">
              <ShieldCheck size={16} className="text-emerald-600"/> 2-Year Full Warranty
            </span>
            <span>•</span>
            <span className="flex items-center gap-1.5 font-bold">
              <Award size={16} className="text-amber-600"/> ISO Certified Quality
            </span>
          </div>
        </div>
        ...
        <div className="relative flex justify-center items-center">
          <div className="absolute w-72 h-72 md:w-96 md:h-96 bg-amber-300/25 rounded-full blur-2xl"></div>
          <img
            key={slide.image}
            src={slide.image}
            alt={slide.alt}
          ...
          <button 
            onClick={prevSlide} 
            className="absolute left-0 z-20 bg-white/90 hover:bg-amber-400 hover:text-black text-zinc-900 p-2.5 rounded-full border border-zinc-200 transition shadow-md"
          ...
          <button 
            onClick={nextSlide} 
            className="absolute right-0 z-20 bg-white/90 hover:bg-amber-400 hover:text-black text-zinc-900 p-2.5 rounded-full border border-zinc-200 transition shadow-md"
        ...
        {bannerSlides.map((_, index) => (
          <button
            ...
            className={`h-2 rounded-full transition-all duration-300 ${
              currentSlide === index ? 'w-8 bg-amber-400' : 'w-2 bg-zinc-300'
            }`}
